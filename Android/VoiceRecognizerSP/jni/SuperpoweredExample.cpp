...
//wahib
/*void SuperpoweredExample::onFFT(float *real, float *imag) {
	__android_log_print(ANDROID_LOG_VERBOSE, "SuperpoweredExample", "onFFT called");

	if(real != NULL && imag != NULL)
	{
		__android_log_print(ANDROID_LOG_VERBOSE, "SuperpoweredExample", "onFFT real & imag is not null");
        jsize len = env->GetArrayLength(*real);
		printf("My_float_ptr is %p\n", real);
	}
	else
		__android_log_print(ANDROID_LOG_VERBOSE, "SuperpoweredExample", "onFFT real or imag is NULL");


	 int logSize = 9; //2^9 = 512 = FFTSize
	 bool ifReal = true;
	 bool ifForward = false; //false = inverse

	 //SuperpoweredFFTPrepare(logSize, ifReal);
	 //SuperpoweredFFTReal(real, imag, logSize, ifForward);
...
void SuperpoweredExample::onFFTReal(JNIEnv *env, jobject obj, jfloatArray real, jfloatArray imag,  int logSize, bool ifForward) {
	//__android_log_print(ANDROID_LOG_VERBOSE, "SuperpoweredExample", "native onFFT called");

    float* nativeReal = (float *)env->GetFloatArrayElements(real, 0);
    float* nativeImag = (float *)env->GetFloatArrayElements(imag, 0);


	if(nativeReal != NULL && nativeImag != NULL )
	{
		SuperpoweredFFTReal(nativeReal, nativeImag, logSize, ifForward);

		//__android_log_print(ANDROID_LOG_VERBOSE, "SuperpoweredExample", "onFFT real & imag is not null");
        //jsize len = sizeof(nativeReal);
		//__android_log_print(ANDROID_LOG_VERBOSE, "SuperpoweredExample", "real size is %d", len);


		//__android_log_print(ANDROID_LOG_INFO, "SuperpoweredExample", "Real array values = %f, %f, %f",	nativeReal[0], nativeReal[1], nativeReal[2]);
		//__android_log_print(ANDROID_LOG_INFO, "SuperpoweredExample", "Imaginary array values = %f, %f, %f",	nativeImag[0], nativeImag[1], nativeImag[2]);
	...
	env->ReleaseFloatArrayElements(real, nativeReal, 0);
	env->ReleaseFloatArrayElements(imag, nativeImag, 0);
